...
public class Main {
    public static void main(String[] args) throws SQLException {
        try (Connection conn = ConnectionFactory.getConnection())
        {
            ...
            dao.add(c);
            for (int i = 0; i < 10; i++) {
                Client cl = new Client("Client" + i, 20 + i);
                dao.add(cl);
            }
            // int id = c.getId();

            List<Client> list = dao.getAll(Client.class);
            for (Client cli : list)  System.out.println(cli);

            //list.get(0).setAge(55);
            //dao.update(list.get(0));

            List<Client> llll = dao.getAll(Client.class, "name", "age");
            for (Client cli : llll)  System.out.println(cli);

            List<Client> tttt = dao.getAll(Client.class, "age");
            for (Client cli : tttt)  System.out.println(cli);

            List<Client> tttt2 = dao.getAll(Client.class, "name");
            for (Client cli : tttt2)  System.out.println(cli);
            /*
